    ...
    TimeProvider timeProvider) : IMembershipRepository
{
        ...
        return context.Memberships.AsNoTracking()
            .SingleOrDefaultAsync(
                x =>
                    ...
                    (!x.EndsAtUtc.HasValue || x.EndsAtUtc > utcNow),
                cancellationToken);
    }
    ...
    public Task<Membership?> GetMembershipForUserAndPlanAsync(
        ...
        context.Memberships
            .Include(x => x.History)
            .SingleOrDefaultAsync(
                x => x.UserId == userId && x.PlanCode == planCode,
                cancellationToken);
    ...
    public async Task<(IReadOnlyCollection<MembershipHistoryResponse> Items, int TotalCount)> GetHistoryAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var source = context.MembershipHistories
            .AsNoTracking()
            .Where(x => x.UserId == userId);
        ...
        var items = await source
            ...
            .Take(query.PageSize)
            .Select(x => new MembershipHistoryResponse(
                x.Id,
                x.MembershipId,
                x.PreviousStatus,
                x.CurrentStatus,
        ...
        CancellationToken cancellationToken = default)
    {
        if (await context.UserJobHistories.AnyAsync(
                x =>
                    x.UserId == userId &&
                    x.JobId == jobId &&
                    x.Action == JobHistoryAction.Applied,
                cancellationToken))
        {
            return;
        }

        await context.UserJobHistories.AddAsync(
            new UserJobHistory
            {
                UserId = userId,
                JobId = jobId,
...
public sealed class PaymentRepository(JobPortalDbContext context) : IPaymentRepository
{
    ...
    public Task<Payment?> GetOwnedByProviderOrderIdAsync(
        ...
        context.Payments
            ...
            .SingleOrDefaultAsync(
                x =>
                    x.ProviderOrderId == providerOrderId &&
                    x.UserId == userId,
                cancellationToken);
    ...
    public Task<bool> HasProcessedProviderEventAsync(
        ...
        CancellationToken cancellationToken = default) =>
        context.PaymentHistories
            .IgnoreQueryFilters()
            .AsNoTracking()
            .AnyAsync(
                x => x.ProviderEventId == providerEventId,
                cancellationToken);
    ...
    public async Task<(IReadOnlyCollection<PaymentHistoryResponse> Items, int TotalCount)> GetHistoryAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var source = context.PaymentHistories
            .AsNoTracking()
            .Where(x => x.UserId == userId);
