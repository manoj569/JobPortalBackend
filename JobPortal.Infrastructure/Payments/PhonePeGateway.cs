...
public sealed class PhonePeGateway(
    ...
    PhonePeAccessTokenCache tokenCache) : IPhonePeGateway
{
        ...
        string merchantOrderId, CancellationToken cancellationToken = default)
    {
        using var request = new HttpRequestMessage(HttpMethod.Get,
            $"checkout/v2/order/{Uri.EscapeDataString(merchantOrderId)}/status?details=true");
        using var response = await SendAuthorizedAsync(request, cancellationToken);
        var body = await response.Content.ReadAsStringAsync(cancellationToken);
        if (!response.IsSuccessStatusCode)
        ...
        try
        {
            ...
            var returnedMerchantOrderId = root.TryGetProperty("merchantOrderId", out var merchantId)
                ? merchantId.GetString() : root.GetProperty("orderId").GetString();
            if (!string.Equals(returnedMerchantOrderId, merchantOrderId, StringComparison.Ordinal))
                throw new JsonException();
            var state = ParseState(root.GetProperty("state").GetString());
            long? amount = root.TryGetProperty("amount", out var amountElement) && amountElement.TryGetInt64(out var value)
                ? value : null;
            string? transactionId = null;
            if (root.TryGetProperty("paymentDetails", out var details) && details.ValueKind == JsonValueKind.Array)
                transactionId = details.EnumerateArray().Select(x =>
                    x.TryGetProperty("transactionId", out var id) ? id.GetString() : null)
                    .LastOrDefault(x => !string.IsNullOrWhiteSpace(x));
            return new(state, merchantOrderId, transactionId, amount);
        }
        catch (JsonException)
        {
