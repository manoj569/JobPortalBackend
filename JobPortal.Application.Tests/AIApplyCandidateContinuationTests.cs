...
public sealed class AIApplyCandidateContinuationTests
{
    ...
    {
        var names = typeof(AIApplyApplicationResponse).GetProperties().Concat(typeof(CandidateExternalApplicationResponse).GetProperties()).Select(x => x.Name).ToArray();
    ...
    private static (AIApplyService Service, FakeRepository Repository, AIApplyApplication Application) Fixture(AIApplyRunStatus status, AIApplyFailureKind failure, bool safeLink = true, bool authorized = true)
    {
        ...
        var application = new AIApplyApplication { UserId = userId, JobId = jobId, Status = status, FailureKind = failure, ScheduledAtUtc = Now.AddMinutes(-5), ExternalApplicationUrl = "https://boards.greenhouse.io/acme/jobs/123", NormalizedApplicationUrl = "https://boards.greenhouse.io/acme/jobs/123" };
        var repository = new FakeRepository(application, new Job { Id = jobId, Status = JobStatus.Published, ExpiresAtUtc = Now.AddDays(2), ApplicationUrl = application.ExternalApplicationUrl });
    ...
    private sealed class FakeRepository(AIApplyApplication application, Job job) : IAIApplyRepository
    {
        ...
        public Task<User?> GetUserProfileAsync(Guid userId, CancellationToken ct = default) => Task.FromResult<User?>(new User { Id = userId, Status = UserStatus.Active });
        ...
        public Task<AIApplySetting?> GetSettingsAsync(Guid userId, CancellationToken ct = default) => Task.FromResult<AIApplySetting?>(null);
        ...
        public Task AddAsync<T>(T entity, CancellationToken ct = default) where T : class => Task.CompletedTask;
