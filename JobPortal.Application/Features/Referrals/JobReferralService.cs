    ...
    IDashboardRepository? dashboard = null) : IJobReferralService
{
        ...
        Guid referrerUserId, SubmitJobReferralRequest request, CancellationToken cancellationToken = default)
    {
        if (!request.ShowLinkedIn && !request.ShowEmail && !request.ShowPhone)
            throw new BadRequestException(
                "Select at least one contact field to share with job seekers.", "validation_error");
        ...
        var composed = await jobService.ComposeAsync(referrerUserId, request.JobDetails, cancellationToken);
        ...
        var job = await jobs.GetByIdAsync(composed.Id, false, cancellationToken)
    ...
    {
        var (items, totalCount) = await referrals.GetPendingAsync(pageNumber, pageSize, cancellationToken);
        var responses = new List<JobReferralResponse>(items.Count);
        foreach (var referral in items)
            responses.Add(await ToResponseAsync(referral, referral.Job, cancellationToken));

        return new PagedResponse<JobReferralResponse>(responses, pageNumber, pageSize, totalCount);
    }
    ...
    {
        var referral = await referrals.GetByIdAsync(jobReferralId, cancellationToken)
            ?? throw new NotFoundException("Referral was not found.");

        if (referral.ApprovalStatus != JobReferralApprovalStatus.Pending)
            throw new ConflictException("This referral has already been reviewed.");

        if (request.Decision == JobReferralApprovalStatus.Rejected &&
            string.IsNullOrWhiteSpace(request.RejectionReason))
            throw new BadRequestException("A rejection reason is required.", "validation_error");

        referral.ApprovalStatus = request.Decision;
        referral.ReviewedByUserId = adminUserId;
        referral.ReviewedAtUtc = UtcNow;
        referral.RejectionReason = request.Decision == JobReferralApprovalStatus.Rejected
            ? request.RejectionReason
            : null;

        var job = await jobs.GetByIdAsync(referral.JobId, false, cancellationToken)
            ?? throw new NotFoundException("Job was not found.");
        ...
        if (request.Decision == JobReferralApprovalStatus.Approved)
        {
            ...
            jobs.Update(job);
            if (dashboard is not null && !await dashboard.NotificationExistsAsync(referral.ReferrerUserId, NotificationType.ReferralApproved, referral.Id, cancellationToken))
            {
                await dashboard.AddNotificationAsync(new Notification
                {
        ...
        await auditWriter.AppendAsync(new(
            request.Decision == JobReferralApprovalStatus.Approved ? AuditAction.Publish : AuditAction.Update,
            "JobReferral",
            referral.Id.ToString(),
            new Dictionary<string, string?>
            {
        ...
        await unitOfWork.SaveChangesAsync(cancellationToken);
        return await ToResponseAsync(referral, job, cancellationToken);
    }
    ...
    {
        var items = await referrals.GetByReferrerAsync(referrerUserId, cancellationToken);
        var responses = new List<JobReferralResponse>(items.Count);
        foreach (var referral in items)
            responses.Add(await ToResponseAsync(referral, referral.Job, cancellationToken));
        return responses;
    ...
    public async Task<PagedResponse<PublicReferralJobResponse>> GetApprovedPublicAsync(
        int pageNumber, int pageSize, CancellationToken cancellationToken = default)
    {
        var (items, totalCount) = await referrals.GetApprovedAsync(pageNumber, pageSize, cancellationToken);
        var responses = items.Select(referral => new PublicReferralJobResponse(
            referral.JobId,
            referral.Job.Title,
            referral.Job.Company.Name,
            referral.Job.Location,
            $"{referral.ReferrerUser.FirstName} {referral.ReferrerUser.LastName}".Trim())).ToList();

        return new PagedResponse<PublicReferralJobResponse>(responses, pageNumber, pageSize, totalCount);
    }
    ...
    {
        var referral = await referrals.GetByJobIdAsync(jobId, cancellationToken)
            ?? throw new NotFoundException("This job has no referral attached.");

        if (referral.ApprovalStatus != JobReferralApprovalStatus.Approved)
            throw new NotFoundException("This job has no referral attached.");

        if (!seekerUserId.HasValue)
            return new ReferralUnlockResponse(ReferralUnlockStatus.LoginRequired, "Please log in to continue.");

        var membership = await memberships.GetActiveForUserAsync(seekerUserId.Value, cancellationToken);
        if (membership is null)
            return new ReferralUnlockResponse(
                ReferralUnlockStatus.MembershipRequired,
                "Subscribe for ₹299/month to unlock referrer contact details.");

        await referrals.RecordUnlockAsync(referral.Id, seekerUserId.Value, cancellationToken);
        await unitOfWork.SaveChangesAsync(cancellationToken);

        var referrerName = $"{referral.ReferrerUser.FirstName} {referral.ReferrerUser.LastName}".Trim();
        var contact = new ReferrerContactDetailsResponse(
            referrerName,
            referral.ShowLinkedIn ? referral.ReferrerUser.LinkedInUrl : null,
            referral.ShowEmail ? referral.ReferrerUser.Email : null,
            referral.ShowPhone ? referral.ReferrerUser.PhoneNumber : null);

        return new ReferralUnlockResponse(ReferralUnlockStatus.Granted, "Contact details unlocked.", contact);
    }

    private static Task<JobReferralResponse> ToResponseAsync(
        JobReferral referral, Job job, CancellationToken cancellationToken)
    {
        var referrerName = $"{referral.ReferrerUser.FirstName} {referral.ReferrerUser.LastName}".Trim();
        return Task.FromResult(new JobReferralResponse(
            referral.Id,
            referral.JobId,
            job.Title,
